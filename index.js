...
function returnProfil(){ /*function apeler lors du click sur le lien "Retourner au menu principal"  */
...
function buttonProfil(){ /*function appeler lors du click sur le lien profil */
    
    var request = new XMLHttpRequest();
    request.onreadystatechange = function() { /* requette GET qui recupere les données de la table profil via le pseudo de l'utilisateur*/
   ...
   const  tokens = sessionStorage.getItem('token')
    ...
    request.open("GET", "http://localhost:3000/api/profil/getprofil/" + userIdProfil);
    request.setRequestHeader('Content-Type' , 'application/json')
    request.setRequestHeader('Authorization', ` Bearer ${token}`);
      request.send();
  }
function displayUpdateProfil(){ /* function apeler lors du click sur le bouton Modifier votre Profil*/
  ...
  function modifyProfil(){ /* function apeler lors du click sur le bouton valider les modification  */
 

    var request = new XMLHttpRequest();
   request.onreadystatechange = function() { /* requette PUT qui va envoyer a la BDD les modification apporter au profil */
   ...
   request.open("PUT", "http://localhost:3000/api/updateprofil/" + userIdProfil);
   const  tokens = sessionStorage.getItem('token')
    var token = JSON.parse(tokens)
    request.setRequestHeader('Authorization', ` Bearer ${token}`);
...
function displayFormPost(){ /* function apeler lors du click sur le lien post */
...
function post(){ /* function apeler lors du click sur le bouton "partager" */ 
   

    var request = new XMLHttpRequest();
        request.onreadystatechange = function() { /*  requette post qui envoie dans la base de donnéer un fichier multimedia est le pseudo */
            if (this.readyState == XMLHttpRequest.DONE && this.status == 201) {
                var response = JSON.parse(request.responseText);
 ...
 // tableaux pour recuperer l'id du post 
  ...
  function getALLpost() { /* function apeler lorsque la page se recharge est lors de la réussite de la requette post post */

        var request = new XMLHttpRequest();
        request.onreadystatechange = function() { /* requette GET qui affiche tout les post in html */
        if (this.readyState == XMLHttpRequest.DONE && this.status == 200) {
...
console.log(recupCom)
    ...
    request.open("GET", "http://localhost:3000/api/post/forum");
...
// recupération de l'id dans l'url pour pouvoir faire une requette get rien qu'avec l'id du post selectionner
        ...
        function deleteUser(){ /* function apeler lors du click surl le lien "supprimer mon compte"*/ 
